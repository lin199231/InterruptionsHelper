...
public class InterruptionsHelper extends Activity {
    ...
    private InterruptionsHelperFragment getSelectedFragment() {
        return (InterruptionsHelperFragment) mTabsAdapter.getItem(getRtlPosition(mSelectedTab));
    }
    ...
    private void createTabs(int selectedIndex) {
        ...
        if (mActionBar != null) {
            ...
            mTabsAdapter.addTab(mAlarmTab, InterruptionsFragment.class, ALARM_TAB_INDEX);

            mClockTab = mActionBar.newTab();
            mClockTab.setIcon(R.drawable.clock_tab);
            mClockTab.setContentDescription(R.string.menu_clock);
            mTabsAdapter.addTab(mClockTab, InterruptionsFragment.class, CLOCK_TAB_INDEX);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            if (mActionBar.getSelectedNavigationIndex() == ALARM_TAB_INDEX ||
                ...
                menu.clear();
                getMenuInflater().inflate(R.menu.interruptions_helper_menu, menu);
            }
    ...
    private boolean processMenuClick(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_item_night_mode:
                startActivity(new Intent(InterruptionsHelper.this, ScreensaverActivity.class));
            default:
                break;
        }
        */
    ...
    private void setHomeTimeZone() {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        String homeTimeZone = prefs.getString(SettingsFragment.KEY_HOME_TZ, "");
        if (!homeTimeZone.isEmpty()) {
            return;
        }
       // homeTimeZone = TimeZone.getDefault().getID();
        SharedPreferences.Editor editor = prefs.edit();
        editor.putString(SettingsFragment.KEY_HOME_TZ, homeTimeZone);
        editor.apply();
        Log.v(LOG_TAG, "Setting home time zone to " + homeTimeZone);
    }

    public void registerPageChangedListener(InterruptionsHelperFragment frag) {
        if (mTabsAdapter != null) {
            mTabsAdapter.registerPageChangedListener(frag);
        }
    }

    public void unregisterPageChangedListener(InterruptionsHelperFragment frag) {
        if (mTabsAdapter != null) {
        ...
        */
        ...
        public void onTabSelected(Tab tab, FragmentTransaction ft) {
            ...
            if (mIsFirstLaunch && isClockTab(rtlSafePosition)) {
                ...
                //InterruptionsHelperFragment f = (InterruptionsHelperFragment) getItem(rtlSafePosition);
               // f.setFabAppearance();
        ...
        private void notifyPageChanged(int newPage) {
            for (String tag : mFragmentTags) {
                ...
                InterruptionsHelperFragment f = (InterruptionsHelperFragment) fm.findFragmentByTag(tag);
                if (f != null) {
